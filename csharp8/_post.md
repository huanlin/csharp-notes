...
想像一下，你正在開發一個線上購物網站，當使用者在瀏覽商品清單頁面時，拚命地快速往下滑（快速跳下一頁），此時你的網站應用程式會收到許多抓取資料的請求。假設使用者每次往下跳一頁需要抓取 50 筆資料，那麼當使用者連續快速跳下一頁，雖然她的鍵盤與滑鼠都還可以繼續操作，但終究還是得等到所有該抓的資料全部取得之後，才有辦法看到她想看的東西。換言之，對使用者而言，無論我們的網站後端程式有沒有在背後使用 `async` 語法，她等待的時間是一樣的，因為她的目的只有一個：要盡快看到她想看的資料，而不是在查詢資料時還去點其他按鈕或連結。
...
此範例的功能很單純，只是呼叫 `GetNumbers` 來取得一串數字，然後用一個迴圈把數字輸出至螢幕。執行結果如下：
...
剛才的範例程式雖然能夠一邊讀取串列元素，一邊立刻將取得之元素內容輸出至螢幕上，而且程式流程反覆跳來跳去，感覺有點像非同步呼叫，但並不是；剛才的範例都是同步呼叫。現在試著想像一下，`GetNumbers` 方法裡面會呼叫其他 Web API 或資料查詢函式，因而必須使用 async 呼叫，像這樣：
